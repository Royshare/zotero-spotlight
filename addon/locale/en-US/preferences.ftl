...
pref-priority-hint = Choose which libraries are searched by default. Unchecked libraries are excluded from results, but you can still reach everything from the search itself - `:col query` always searches every collection in every library. Optionally keep deselected libraries searchable at a lower rank. Tune word matching with "matching": {"mode": "field", "typoDistance": 1, "minTokenLength": 4}; set mode to "loose" for the older whole-record fuzzy behavior.
...
pref-matching-label = Word Matching
pref-matching-mode = Matching mode
pref-matching-mode-field = Field (whole words, typo-tolerant)
pref-matching-mode-loose = Loose (legacy whole-record fuzzy)
pref-matching-typo = Typo tolerance
pref-matching-minlen = Shortest word typos apply to
